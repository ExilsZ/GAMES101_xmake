...
static auto insideTriangle(int x, int y, const Vector4f* _v) -> bool {
    Vector3f v[3];
    ...
    f2 = v[0].cross(v[2]);
    Vector3f p(x, y, 1.);
    return (p.dot(f0) * f0.dot(v[2]) > 0) && (p.dot(f1) * f1.dot(v[0]) > 0) &&
           (p.dot(f2) * f2.dot(v[1]) > 0);
}

static auto computeBarycentric2D(float x, float y,
                                 const Vector4f* v) -> std::tuple<float, float, float> {
...
void rst::rasterizer::draw(std::vector<Triangle*>& TriangleList) {
    ...
    for (const auto& t : TriangleList) {
        ...
        std::array<Eigen::Vector4f, 3> mm{(view * model * t->v[0]), (view * model * t->v[1]),
                                          (view * model * t->v[2])};
...
static auto interpolate(float alpha, float beta, float gamma, const Eigen::Vector3f& vert1,
                        const Eigen::Vector3f& vert2, const Eigen::Vector3f& vert3,
                        float weight) -> Eigen::Vector3f {
    return (alpha * vert1 + beta * vert2 + gamma * vert3) / weight;
}

static auto interpolate(float alpha, float beta, float gamma, const Eigen::Vector2f& vert1,
                        const Eigen::Vector2f& vert2, const Eigen::Vector2f& vert3,
                        float weight) -> Eigen::Vector2f {
        ...
        for (int y = ymin; y <= ymax; y++) {
            if (insideTriangle(x + 0.5, y + 0.5, t.v)) {
                ...
                if (zp < depth_buf[get_index(x, y)]) {
                    ...
                    FragmentShaderPayload payload(interpolated_color, interpolated_normal,
...
rst::rasterizer::rasterizer(int w, int h) : texture(std::nullopt), width(w), height(h) {
...
void rst::rasterizer::set_vertex_shader(
    std::function<Eigen::Vector3f(VertexShaderPayload)> vert_shader) {
    vertex_shader = vert_shader;
}

void rst::rasterizer::set_fragment_shader(
    std::function<Eigen::Vector3f(FragmentShaderPayload)> frag_shader) {
