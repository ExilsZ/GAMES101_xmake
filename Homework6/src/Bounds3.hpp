...
class Bounds3 {
  public:
    ...
    [[nodiscard]] inline auto IntersectP(const Ray& ray, const Vector3f& invDir,
                                         const std::array<int, 3>& dirisNeg) const -> bool;
};

// 判断 BoundingBox 是否与光线相交
inline auto Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir,
    ...
    // than Division
    // dirIsNeg: ray direction(x,y,z), dirIsNeg=[int(x>0),int(y>0),int(z>0)], use this
