...
auto Scene::castRay(const Ray& ray, int depth) const -> Vector3f {
    if (depth > this->maxDepth) { return {0.0, 0.0, 0.0}; }
    ...
    if (intersection.happened) {
        ...
        default: {
            ...
            Vector3f shadowPointOrig = (dotProduct(ray.direction, N) < 0) ? hitPoint + N * EPSILON
                                                                          : hitPoint - N * EPSILON;
            // [comment]
            ...
            for (uint32_t i = 0; i < get_lights().size(); ++i) {
                ...
                if (area_ptr != nullptr) {
                    ...
                    specularColor +=
                        powf(std::max(0.f, -dotProduct(reflectionDirection, ray.direction)),
                             m->specularExponent) *
