...
#    define TIXMLASSERT(x)                                                                         \
        if (!((void)0, (x))) { __debugbreak(); } // if ( !(x)) WinDebugBreak()
#  elif defined(ANDROID_NDK)
#    include <android/log.h>
#    define TIXMLASSERT(x)                                                                         \
        if (!(x)) {                                                                                \
            __android_log_assert("assert", "grinliz", "ASSERT in '%s' at %d.", __FILE__,           \
                                 __LINE__);                                                        \
        }
#  else
#    include <assert.h>
...
namespace tinyxml2 {
      ...
      public:
        ...
        virtual void* Alloc() {
            if (!_root) {
                ...
                for (int i = 0; i < COUNT - 1; ++i) { block->chunk[i].next = &block->chunk[i + 1]; }
            ...
            ++_currentAllocs;
            if (_currentAllocs > _maxAllocs) { _maxAllocs = _currentAllocs; }
            _nAllocs++;
            _nUntracked++;
            return result;
        }
        ...
        void Trace(const char* name) {
            printf("Mempool %s watermark=%d [%dk] current=%d size=%d nAlloc=%d blocks=%d\n", name,
                   _maxAllocs, _maxAllocs * SIZE / 1024, _currentAllocs, SIZE, _nAllocs,
                   _blockPtrs.Size());
        }
    ...
    class XMLUtil {
      public:
        ...
        inline static bool StringEqual(const char* p, const char* q, int nChar = INT_MAX) {
            ...
            if ((n == nChar) || (*p == 0 && *q == 0)) { return true; }
            return false;
        }
    ...
    class TINYXML2_LIB XMLElement : public XMLNode {
      ...
      public:
        ...
        */
        ...
        /// Convenience method for setting text inside and element. See SetText() for important
        /// limitations.
    ...
    */
    ...
    /**
            ...
            XMLElement* child2 = docHandle.FirstChildElement( "Document" ).FirstChildElement(
       "Element" ).FirstChildElement().NextSiblingElement(); if ( child2 )
            {
                    // do something useful
            @endverbatim
    ...
    */
    ...
    /**
            A variant of the XMLHandle class for working with const XMLNodes and Documents. It is
       the same in all regards, except for the 'const' qualifiers. See XMLHandle for API.
    */
