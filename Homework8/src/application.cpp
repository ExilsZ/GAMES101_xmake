...
namespace CGL {
    ...
    void Application::init() {
        ...
        // Create two ropes
        ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), 7, config.mass, config.ks, {0});
        ropeVerlet =
            new Rope(Vector2D(0, 200), Vector2D(-400, 200), 7, config.mass, config.ks, {0});
    }
    ...
    void Application::keyboard_event(int key, int event, unsigned char mods) {
        switch (key) {
            case '-':
                if (config.steps_per_frame > 1) { config.steps_per_frame /= 2; }
                break;
            case '=': config.steps_per_frame *= 2; break;
        }
    }
